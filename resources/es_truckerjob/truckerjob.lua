...
TruckingCompany[0] = {["x"] = -406.421966552734,["y"] = -2794.29443359375, ["z"] = 6.00038003921509}
local TruckingTrailer = {}
TruckingTrailer[0] = {["x"] = -533.353454589844, ["y"] = -2820.33642578125, ["z"] = 6.00038576126099}

local Truck = {"HAULER", "PACKER", "PHANTOM"}
local Trailer = {"TANKER", "TRAILERS", "TRAILERS2", "TRAILERLOGS"}

local MissionData = {
    [0] = {594.761291503906, -2770.05224609375, 6.05681467056274, 1200}, --x,y,z,money
    [1] = {2574.5144, 328.5554, 108.45, 2300},
    [2] = {602.009521484375, 268.625, 103.472854614258, 3400},
    [3] = {650.879333496094, 2768.23828125, 41.9326171875, 4500},
    [4] = {2136.423828125, 4778.421875, 40.9705619812012, 6000},
    [5] = {-325.537719726563, -2763.81469726563, 5.00023508071899, 1}
}
local MISSION = {}
MISSION.start = false
...
function tick()
    ...
    elseif( MISSION.start == true ) then
        ...
        if( IsEntityAttached(MISSION.trailer) and text1 == false) then
            TriggerEvent("mt:missiontext", "Rendez-vous au ~g~point~w~ par votre GPS.", 10000)
            text1 = true
        ...
        if ( GetDistanceBetweenCoords(currentMission[1], currentMission[2], currentMission[3], trailerCoords ) < 25 and  not IsEntityAttached(MISSION.trailer)) then
            TriggerEvent("mt:missiontext", "Vous avez gagné la somme de $"..currentMission[4], 5000)
        ...
        elseif ( GetDistanceBetweenCoords(currentMission[1], currentMission[2], currentMission[3], trailerCoords ) < 25 and IsEntityAttached(MISSION.trailer) ) then
            TriggerEvent("mt:missiontext", "Vous êtes arrivé. Détachez la ~o~remorque~w~ en restant appuyé sur ~r~H~w~", 15000)
        end
...
function MISSION.spawnTruck()
    ...
    MISSION.truck = CreateVehicle(MISSION.hashTruck, -444.805511474609, -2791.45458984375, 6.00038290023804, 0.0, true, false)
...
function GUI.drawStartText()
    TriggerEvent("mt:missiontext", "Metier de routier, appuyez sur ~r~N+~w~ pour commencer.", 500)
...
function GUI.init()
    GUI.loaded = true
    GUI.addTitle("Vous êtes routier à présent.", 0.425, 0.19, 0.45, 0.07 )
    GUI.addDesc("Choisssez votre transport.", 0.575, 0.375, 0.15, 0.30 )

    --menu, title, function, position
    GUI.addButton(0, "Transport Citerne RON", GUI.optionMisson, 0.35, 0.25, 0.3, 0.05 )
    GUI.addButton(0, "Transport de Container", GUI.optionMisson, 0.35, 0.30, 0.3, 0.05 )
    GUI.addButton(0, "Transprot Articulé (???)", GUI.optionMisson, 0.35, 0.35, 0.3, 0.05 )
    GUI.addButton(0, "Transport de bois", GUI.optionMisson, 0.35, 0.40, 0.3, 0.05 )
    GUI.addButton(0, " ", GUI.null, 0.35, 0.45, 0.3, 0.05)
    GUI.addButton(0, "Sortir du Menu", GUI.exit, 0.35, 0.50, 0.3, 0.05 )
    
    GUI.buttonCount = 0
    
    GUI.addButton(1, "Mission 1 [ 1.200$ ]", GUI.mission, 0.35, 0.25, 0.3, 0.05)
    GUI.addButton(1, "Mission 2 [ 2.300$ ]", GUI.mission, 0.35, 0.30, 0.3, 0.05)
    GUI.addButton(1, "Mission 3 [ 3.400$ ]", GUI.mission, 0.35, 0.35, 0.3, 0.05)
    GUI.addButton(1, "Mission 4 [ 4.500$ ]", GUI.mission, 0.35, 0.40, 0.3, 0.05)
    GUI.addButton(1, "Mission 5 [ 6.000$ ]", GUI.mission, 0.35, 0.45, 0.3, 0.05)
    GUI.addButton(1, "Test [ 1$ ]", GUI.mission, 0.35, 0.50, 0.3, 0.05)
    GUI.addButton(1, "Sortir du Menu", GUI.exit, 0.35, 0.55, 0.3, 0.05)
end
